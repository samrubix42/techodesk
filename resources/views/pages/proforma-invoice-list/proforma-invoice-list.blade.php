...
<div class="space-y-6 px-3 py-5 sm:px-4 sm:py-6 lg:px-6">
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        ...
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
            View and download proforma invoices.
        </p>
    </div>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <h2 class="mb-3 text-lg font-semibold text-slate-900 dark:text-white">Proforma Invoices</h2>
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody>
                    @forelse ($this->proformaInvoices as $invoice)
                        <tr class="border-b border-slate-100 dark:border-slate-900">
                            ...
                            <td class="px-2 py-2">
                                <a href="{{ route('invoice.pdf', ['type' => 'proforma', 'invoice' => $invoice->id]) }}" class="inline-flex rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-medium text-white dark:bg-white dark:text-slate-900">
                                    PDF
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="4" class="px-2 py-4 text-slate-500">No proforma invoices found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
